...
export const createTicket = async(ticketData, user) => {
    ...
    if (ticketData.category_name) {
        const res = await pool.query("SELECT category_id FROM ticket_categories WHERE category_name = $1 AND company_id = $2", [ticketData.category_name, user.companyId]);
        if (res.rows.length > 0) category_id = res.rows[0].category_id;
    }
    if (ticketData.priority_name) {
        const res = await pool.query("SELECT priority_id FROM ticket_priorities WHERE priority_name = $1 AND company_id = $2", [ticketData.priority_name, user.companyId]);
        if (res.rows.length > 0) priority_id = res.rows[0].priority_id;
    }
    if (ticketData.status_name) {
        const res = await pool.query("SELECT status_id FROM ticket_statuses WHERE status_name = $1 AND company_id = $2", [ticketData.status_name, user.companyId]);
        if (res.rows.length > 0) status_id = res.rows[0].status_id;
    }

    // Fallbacks
    if (!category_id) {
        const res = await pool.query("SELECT category_id FROM ticket_categories WHERE company_id = $1 LIMIT 1", [user.companyId]);
        category_id = res.rows[0]?.category_id;
    }
    if (!priority_id) {
        const res = await pool.query("SELECT priority_id FROM ticket_priorities WHERE company_id = $1 LIMIT 1", [user.companyId]);
        priority_id = res.rows[0]?.priority_id;
    }
    if (!status_id) {
        const res = await pool.query("SELECT status_id FROM ticket_statuses WHERE company_id = $1 AND is_default = true LIMIT 1", [user.companyId]);
...
export const updateTicketStatus = async (ticketId, statusId, user) => {
    ...
    const isTechOrAdmin = Number(user.roleId) === 1 || Number(user.roleId) === 3;
    if (!isTechOrAdmin) {
        const isCreator = ticket.raised_by_user_code === user.userCode;
        const statusRes = await pool.query("SELECT status_name FROM ticket_statuses WHERE status_id = $1", [statusId]);
        const statusName = statusRes.rows[0]?.status_name || "";
        if (isCreator && statusName.toLowerCase() === "closed") {
            // Allowed to close their own ticket
        } else {
            throw new Error("Access denied. Customers can only close their own tickets.");
        }
    }
